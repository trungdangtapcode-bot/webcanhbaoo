...
router.get('/embed', (req, res) => {
  const videoId = sanitizeVideoId(req.query.videoId);

  if (!videoId) {
    return res.status(400).send('Missing or invalid videoId');
  }

  const autoplay = parseFlag(req.query.autoplay, '1');
  const mute = parseFlag(req.query.mute, '1');
  const vq = ['small', 'medium', 'large', 'hd720', 'hd1080'].includes(req.query.vq || '') ? req.query.vq : '';

  const origin = sanitizeAllowedOrigin(req.query.origin, 'http://localhost:3000');
  const parentOrigin = sanitizeAllowedOrigin(req.query.parentOrigin, origin);
...
<body>
  ...
  res.setHeader('permissions-policy', 'storage-access=(self "https://www.youtube.com")');
