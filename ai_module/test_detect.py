...
def main():
    ...
    assert frame is not None, "Frame should not be None at this point"
    ...
    for det in all_detections:
        ...
        color_tuple = (int(color[0]), int(color[1]), int(color[2])) if isinstance(color, tuple) else (255, 255, 255)
        cv2.rectangle(frame, (x1, y1), (x2, y2), color_tuple, 2)
        
        label = f"{cls_name} {conf:.0%}"
        (tw, th), _ = cv2.getTextSize(label, cv2.FONT_HERSHEY_SIMPLEX, 0.6, 2)
        cv2.rectangle(frame, (x1, y1 - th - 10), (x1 + tw + 4, y1), color_tuple, -1)
